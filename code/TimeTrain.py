...
pretrained_net.fc = nn.Linear(512, 9)#因为是9分类
...
def train_fine_tuning(net, optimizer, batch_size=128, num_epochs=50):
    test_imgs = ImageFolder(r'../input/aitus-data/TimeData/test', transform=test_augs)
    test_iter = DataLoader(test_imgs, 1, shuffle=False)
    train_iter = DataLoader(ImageFolder(r'../input/aitus-data/TimeData/train', transform=train_augs), batch_size, shuffle=True)
